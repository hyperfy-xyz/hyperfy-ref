import { createSurface } from './SurfaceNets'
...
export function generateMesh({
  ...
  const surface = createSurface(grid, [gridSize.x, gridSize.y, gridSize.z])

  const topvertmap = {}

  const indices = new Uint16Array(surface.faces.length * 6)
  const vertices = new Float32Array(surface.vertices.length * 3)
  const underground = new Float32Array(surface.vertices.length)
  const adjusted = new Int8Array(surface.vertices.length)
  ...
  for (let i = 0; i < surface.vertices.length; i++) {
    ...
    adjusted[i] =
      adjustedIndices[
        z * (gridSize.x * gridSize.y) + Math.round(y) * gridSize.z + x
      ]

    if (y < terrainHeight) {
      underground[i] =
        y > terrainHeight - smoothRange ? (terrainHeight - y) / smoothRange : 1
    } else {
      underground[i] = 0
