...
export class Actions extends System {
  ...
  update(delta) {
    const avatarPos = this.world.graphics.cameraRig.position

    // clear current action if its no longer in distance
    if (this.current.node) {
      const distance = this.current.node.worldPos.distanceTo(avatarPos)
    ...
    for (let i = 0; i < size; i++) {
      ...
      const distance = node.worldPos.distanceTo(avatarPos)
