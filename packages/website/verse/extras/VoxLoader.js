 ...
 * VoxLoader
 * 
 * This is the same as three 0.164.0 but instead of returning chunks, it returns { scene } with regular meshes.
 * 
 * This makes it more consistent with gltf loader etc
 * 
 */
...
export class VOXLoader extends Loader {
	...
	parse( buffer ) {
        ...
        for (let i = 0; i < chunks.length; i ++) {
            ...
            const mesh = this.buildMesh( chunk, i )
            scene.add(mesh)
	...
	buildMesh( chunk, idx ) {
        ...
        mesh.name = 'vox'
        if (idx > 0) mesh.name += idx

        // scale and use origin as floor
        const scale = 0.05
        mesh.scale.setScalar(scale)
        mesh.position.y = (size.z / 2) * scale
